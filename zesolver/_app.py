...
# Keep the project/package parent on sys.path for source-tree compatibility
# when this module is executed directly (for example macOS preflight). Wheels
# rely on normal installation metadata and site-packages resolution.
if str(_PROJECT_ROOT) not in sys.path:
    sys.path.insert(0, str(_PROJECT_ROOT))


def _package_resource_root() -> Path:
    """Return the package resource root (importlib.resources when available)."""
    try:
...
_PKG_ROOT = _package_resource_root()
...
def _app_log_path() -> Path:
    """XDG-compatible log path; falls back to package-relative location."""
    try:
        base = Path(os.environ.get("ZE_LOG_DIR", ""))
        if not str(base).strip():
            xdg_state = os.environ.get("XDG_STATE_HOME", str(Path.home() / ".local" / "state"))
            base = Path(xdg_state) / "ZeSolver"
        base.mkdir(parents=True, exist_ok=True)
        return base / "zesolver.log"
    except Exception:
        return _PROJECT_ROOT / "zesolver.log"


LOG_FILE = _app_log_path()
APP_ICON_CANDIDATES = (
    "ZSicon.ico",
    "ZSicon.icns",
    "ZSicon.png",
...
def _load_source_version(default: str = "0.0.dev") -> str:
    """Read the version from the shipped pyproject in the wheel, or the source tree."""
    try:
        from importlib.resources import files as _resource_files
        pyproject_text = _resource_files("zesolver").joinpath("pyproject.toml").read_text(encoding="utf-8")
        import tomllib
        ...
        pass
    # Fallback: try source tree pyproject
    try:
        ...
        def _build_database_tab(self) -> QtWidgets.QWidget:
            ...
            def _refresh_distribution_paths() -> None:
                ...
                validation = validate_library_parent(
                    ...
                    application_roots=[_PROJECT_ROOT, *_runtime_resource_dirs()],
                    probe=False,
                )
                if not validation.ok:
